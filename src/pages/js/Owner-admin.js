...
export default function OwnerAdmin() {
  ...
  const fetchSummary = async () => {
    try {
      const { data } = await axios.get(`${BASE_URL}/api/admin/summary`, config);
      setStats({
  ...
  const fetchStores = async () => {
    try {
      const { data } = await axios.get(`${BASE_URL}/api/admin/stores`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        params: {
  ...
  const toggleStoreStatus = async (id, currentStatus) => {
    ...
    try {
      await axios.put(
        `${BASE_URL}/api/admin/stores/${id}/status`,
        { status: newStatus },
        config
      );
      fetchStores();
    ...
    <div className="owner-admin-container">
      ...
      <div className="owner-admin-table-wrapper">
        <div className="owner-admin-header">
          ...
          <button className="owner-admin-logout-btn" onClick={handleLogout}>
            Logout
          </button>
        </div>
        ...
        <div className="owner-admin-controls">
          ...
          <button className="owner-admin-btn" onClick={fetchStores}>
            Apply
          </button>
        </div>
